...
export class CostEstimator implements ICostEstimator {
   ...
   */
  async estimateCosts(trip: TripData, homeLocation: LocationData): Promise<CostEstimate> {
    try {
   ...
   */
  async updateCostEstimate(tripId: string): Promise<CostEstimate> {
    try {
      ...
      (error as any).statusCode = 501; // Not Implemented
      (error as any).details = {
        tripId,
   ...
   */
  private calculateTransportationCosts(destinations: string[], homeLocation: LocationData): CostRange {
    let totalMinCost = 0;
    let totalMaxCost = 0;
   ...
   */
  private calculateFlightCost(from: LocationData | string, to: string | LocationData): CostRange {
   ...
   */
  private determineDistanceCategory(from: string, to: string): 'domestic' | 'regional' | 'intercontinental' {
   ...
   */
  private calculateAccommodationCosts(trip: TripData): CostRange {
   ...
   */
  private calculateActivitiesCosts(trip: TripData): CostRange {
   ...
   */
  private calculateFoodCosts(trip: TripData): CostRange {
    const duration = trip.duration || 7;
    let totalMinCost = 0;
    let totalMaxCost = 0;
   ...
   */
  private calculateTotalCostRange(costRanges: CostRange[]): CostRange {
   ...
   */
  private getLocationRegion(location: string): keyof typeof this.REGIONAL_BASE_COSTS {
   ...
   */
  private isEuropeanDestination(location: string): boolean {
  ...
  private isAsianDestination(location: string): boolean {
    const asianKeywords = [
  ...
  private isAmericasDestination(location: string): boolean {
  ...
  private isAfricanDestination(location: string): boolean {
  ...
  private isOceaniaDestination(location: string): boolean {
   ...
   */
  private isSameCountry(from: string, to: string): boolean {
   ...
   */
  private getDestinationPopularityMultiplier(destination: string): number {
   ...
   */
  private getExperienceCostMultiplier(experiences: string[]): number {
    let multiplier = 1.0;
  ...
  applyTravelStyleAdjustments(baseCost: CostEstimate, travelStyle: 'budget' | 'mid-range' | 'luxury'): CostEstimate {
    ...
    return {
      ...
      total: {
        min: Math.round(
          (baseCost.transportation.min * multipliers.transport) +
        ...
        max: Math.round(
          (baseCost.transportation.max * multipliers.transport) +
