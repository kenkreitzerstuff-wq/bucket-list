...
export class TripPlanner implements ITripPlanner {
  private readonly EXPERIENCE_DURATION_MAP: { [key: string]: number } = {
  ...
  private readonly BASE_DESTINATION_DAYS = 3; // Minimum days per destination
  private readonly TRAVEL_BUFFER_DAYS = 1; // Buffer for travel between destinations

  /**
   * Plans a complete trip with optimized routing and duration calculations
   */
  async planTrip(destinations: string[], preferences: TravelPreferences): Promise<TripPlan> {
    ...
    const destinationPlans = await Promise.all(
      destinations.map(dest => this.createDestinationPlan(dest, preferences))
    );
   ...
   */
  async optimizeRoute(destinations: string[], homeLocation: LocationData | null): Promise<string[]> {
   ...
   */
  private async createDestinationPlan(
    destination: string, 
   ...
   */
  private calculateDestinationDuration(experiences: string[]): number {
   ...
   */
  private async calculateTravelTimes(destinations: string[]): Promise<{ [key: string]: number }> {
   ...
   */
  private groupDestinationsByRegion(destinations: string[]): string[][] {
   ...
   */
  private isEuropeanDestination(dest: string): boolean {
  ...
  private isAsianDestination(dest: string): boolean {
  ...
  private isAmericasDestination(dest: string): boolean {
  ...
  private isAfricanDestination(dest: string): boolean {
  ...
  private isOceaniaDestination(dest: string): boolean {
   ...
   */
  private determineBestTimeToVisit(destination: string): string {
   ...
   */
  private aggregateCostEstimates(destinationPlans: DestinationPlan[]): CostEstimate {
    const aggregated = {
   ...
   */
  private estimateTransportationCost(destination: string, travelStyle: string): CostRange {
    const destLower = destination.toLowerCase();
