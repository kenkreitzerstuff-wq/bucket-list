...
import { BucketListDataService } from './BucketListDataService';
...
export class RecommendationEngine {
   ...
   */
  public static async generateContextualFollowUpQuestions(
    travelInput: TravelInputData,
    userProfile: UserProfile
   ...
   */
  public static async integrateFollowUpAnswers(
  ...
  private static generateBucketListRecommendations(
    ...
    const bucketListItems = BucketListDataService.getRecommendedItems({
    ...
    bucketListItems.slice(0, 5).forEach((item: any) => {
      ...
      recommendations.push({
        ...
        metadata: {
          ...
          difficulty: item.difficulty as 'easy' | 'moderate' | 'challenging'
        }
      });
    });
   ...
   */
  private static calculateBucketListMatchScore(item: any, travelInput: TravelInputData): number {
    let score = 0.5; // Base score

    // Check destination matches
    const destinationMatches = travelInput.destinations.some((dest: string) =>
      item.destination.toLowerCase().includes(dest.toLowerCase()) ||
      dest.toLowerCase().includes(item.destination.toLowerCase().split('(')[0].trim().toLowerCase())
    );
    if (destinationMatches) score += 0.3;

    // Check experience matches
    const experienceMatches = travelInput.experiences.some((exp: string) =>
      item.experiences.some((bucketExp: string) =>
        bucketExp.toLowerCase().includes(exp.toLowerCase()) ||
        exp.toLowerCase().includes(bucketExp.toLowerCase())
      )
    );
    if (experienceMatches) score += 0.2;

    // Check interest/tag matches
    if (travelInput.preferences?.interests) {
      const interestMatches = travelInput.preferences.interests.some((interest: string) =>
        item.tags.some((tag: string) => tag.includes(interest.toLowerCase()))
      );
      if (interestMatches) score += 0.15;
    }

    // Boost high priority items
    if (item.kenPriority <= 2) score += 0.1;
    if (item.gailInterestLevel === 'HIGH') score += 0.15;
   ...
   */
  private static getBucketListDescription(item: any): string {
    const descriptions = [
   ...
   */
  private static generateBucketListReasoning(item: any, travelInput: TravelInputData, matchScore: number): string {
    ...
    const destinationMatch = travelInput.destinations.some((dest: string) =>
      item.destination.toLowerCase().includes(dest.toLowerCase())
    );
    if (destinationMatch) {
    ...
    const experienceMatch = travelInput.experiences.some((exp: string) =>
      item.experiences.some((bucketExp: string) =>
        bucketExp.toLowerCase().includes(exp.toLowerCase())
      )
    );
   ...
   */
  private static extractRelatedTerms(item: any, travelInput: TravelInputData): string[] {
    ...
    item.experiences.slice(0, 2).forEach((exp: string) => related.push(exp));
    
    // Add matching tags
    item.tags.slice(0, 2).forEach((tag: string) => related.push(tag));
   ...
   */
  private static generateExperienceRecommendationsFromBucketList(travelInput: TravelInputData): Recommendation[] {
    const recommendations: Recommendation[] = [];
    
    // Find unique experiences from bucket list that match user interests
    const allBucketItems = BucketListDataService.getIncompleteBucketListItems();
    const uniqueExperiences = new Set<string>();
    
    allBucketItems.forEach((item: any) => {
      item.experiences.forEach((exp: string) => {
        const matchesUserInput = travelInput.experiences.some((userExp: string) =>
   ...
   */
  private static mapDifficultyFromPreferences(preferences?: any): string | undefined {
    if (!preferences?.interests) return undefined;
    
    const interests = preferences.interests.map((i: string) => i.toLowerCase());
    
    if (interests.some((i: string) => ['adventure', 'extreme', 'challenging', 'hiking', 'climbing'].includes(i))) {
      return 'challenging';
    }
    if (interests.some((i: string) => ['moderate', 'active', 'walking', 'cultural'].includes(i))) {
      return 'moderate';
    }
    if (interests.some((i: string) => ['relaxation', 'easy', 'comfort', 'luxury'].includes(i))) {
      return 'easy';
    }
   ...
   */
  private static mapDurationFromPreferences(duration?: string): string | undefined {
   ...
   */
  private static analyzeUserInput(travelInput: TravelInputData): {
    hasAdventureInterest: boolean;
    hasCulturalInterest: boolean;
    hasNatureInterest: boolean;
    hasRelaxationInterest: boolean;
  } {
    const allText = [
   ...
   */
  private static async generateSmartFollowUpQuestions(
    travelInput: TravelInputData,
    userProfile: UserProfile
  ): Promise<FollowUpQuestion[]> {
    ...
    const vagueDestinations = travelInput.destinations.filter((dest: string) =>
      ['europe', 'asia', 'africa', 'america', 'world', 'everywhere'].some(term =>
        dest.toLowerCase().includes(term)
      )
    );

    vagueDestinations.forEach((dest: string, index: number) => {
    ...
    const vagueExperiences = travelInput.experiences.filter((exp: string) =>
      ['adventure', 'fun', 'experience', 'activity', 'something'].some(term =>
        exp.toLowerCase().includes(term) && exp.length < 20
      )
    );
  ...
  private static processFollowUpAnswers(
    ...
    Object.entries(answers).forEach(([questionId, answer]) => {
      if (questionId.includes('europe-clarification')) {
        ...
        const specificDestinations = answerArray.map((region: string) => {
          if (region.includes('Western Europe')) return ['Paris, France', 'Amsterdam, Netherlands', 'Berlin, Germany'];
          if (region.includes('Southern Europe')) return ['Rome, Italy', 'Barcelona, Spain', 'Athens, Greece'];
          if (region.includes('Northern Europe')) return ['Stockholm, Sweden', 'London, UK', 'Copenhagen, Denmark'];
          if (region.includes('Eastern Europe')) return ['Prague, Czech Republic', 'Krakow, Poland', 'Budapest, Hungary'];
          if (region.includes('Mediterranean')) return ['Sicily, Italy', 'Santorini, Greece', 'Mallorca, Spain'];
          return [];
        }).flat();

        // Replace vague destinations with specific ones
        updatedInput.destinations = updatedInput.destinations.map((dest: string) =>
          dest.toLowerCase().includes('europe') ? specificDestinations : [dest]
        ).flat();
      }
      ...
      if (questionId.includes('asia-clarification')) {
        const answerArray = Array.isArray(answer) ? answer : [answer];
        const specificDestinations = answerArray.map((region: string) => {
          if (region.includes('Southeast Asia')) return ['Bangkok, Thailand', 'Ho Chi Minh City, Vietnam', 'Bali, Indonesia'];
          if (region.includes('East Asia')) return ['Tokyo, Japan', 'Seoul, South Korea', 'Beijing, China'];
          if (region.includes('South Asia')) return ['Mumbai, India', 'Kathmandu, Nepal', 'Colombo, Sri Lanka'];
          if (region.includes('Central Asia')) return ['Almaty, Kazakhstan', 'Tashkent, Uzbekistan'];
          if (region.includes('Middle East')) return ['Dubai, UAE', 'Amman, Jordan', 'Tel Aviv, Israel'];
          return [];
        }).flat();

        updatedInput.destinations = updatedInput.destinations.map((dest: string) =>
          dest.toLowerCase().includes('asia') ? specificDestinations : [dest]
        ).flat();
      }

      if (questionId === 'experience-specification') {
        const answerArray = Array.isArray(answer) ? answer : [answer];
        const specificExperiences = answerArray.map((type: string) => {
          if (type.includes('Outdoor adventures')) return ['Hiking mountain trails', 'Scuba diving', 'Wildlife safari'];
          if (type.includes('Cultural immersion')) return ['Local festival participation', 'Traditional craft workshops', 'Community homestays'];
          if (type.includes('Culinary experiences')) return ['Cooking classes with locals', 'Street food tours', 'Wine tasting'];
          if (type.includes('Historical exploration')) return ['Ancient ruins exploration', 'Museum visits', 'Architecture tours'];
          if (type.includes('Relaxation')) return ['Spa treatments', 'Beach relaxation', 'Meditation retreats'];
          if (type.includes('Photography')) return ['Landscape photography', 'Wildlife photography', 'Street photography'];
          if (type.includes('Urban exploration')) return ['Nightlife experiences', 'Local markets', 'Modern art galleries'];
          if (type.includes('Adventure sports')) return ['Rock climbing', 'Bungee jumping', 'Paragliding'];
          return [];
        }).flat();

        // Replace vague experiences with specific ones
        updatedInput.experiences = [
          ...updatedInput.experiences.filter((exp: string) => 
            !['adventure', 'fun', 'experience', 'activity', 'something'].some(term =>
              exp.toLowerCase().includes(term) && exp.length < 20
            )
          ),
      ...
      if (questionId === 'budget-specification') {
        ...
        const selectedBudget = budgetMapping[answer as string];
        if (selectedBudget) {
