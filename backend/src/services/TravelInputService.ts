import { TravelInputData, TravelPreferences, ValidationResult, UserProfile } from '../types';
import { UserProfileStorage } from './LocationService';

/**
 * TravelInputService handles validation and storage of travel input data
 */
export class TravelInputService {
  
  /**
   * Validates travel input data
   */
  public static validateTravelInput(input: TravelInputData): ValidationResult {
    const errors: string[] = [];
    ...
    if (!input.destinations || input.destinations.length === 0) {
      ...
      const invalidDestinations = input.destinations.filter(dest => 
        !dest || dest.trim().length < 2
      );
      ...
      const vagueDestinations = input.destinations.filter(dest =>
        vagueTerms.some(term => dest.toLowerCase().includes(term))
      );
    ...
    if (!input.experiences || input.experiences.length === 0) {
      ...
      const invalidExperiences = input.experiences.filter(exp => 
        !exp || exp.trim().length < 2
      );
      ...
      const vagueExperiences = input.experiences.filter(exp =>
        ['adventure', 'fun', 'experience', 'activity', 'something'].some(term =>
          exp.toLowerCase().includes(term) && exp.toLowerCase().trim().length < 15
        )
      );
   ...
   */
  public static detectIncompleteInput(input: TravelInputData): {
    ...
    const vagueDestinations = input.destinations?.filter(dest =>
      vagueDestinationTerms.some(term => dest.toLowerCase().includes(term))
    ) || [];
    ...
    const vagueExperiences = input.experiences?.filter(exp =>
      vagueExperienceTerms.some(term => 
        exp.toLowerCase().includes(term) && exp.toLowerCase().trim().length < 15
      )
    ) || [];
   ...
   */
  public static generateFollowUpQuestions(input: TravelInputData): Array<{
    id: string;
    question: string;
    ...
    const vagueDestinations = input.destinations?.filter(dest =>
      ['europe', 'asia', 'africa', 'america'].some(term => dest.toLowerCase().includes(term))
    ) || [];
    ...
    const vagueExperiences = input.experiences?.filter(exp =>
      ['adventure', 'fun', 'experience'].some(term => exp.toLowerCase().includes(term))
    ) || [];
   ...
   */
  public static storeTravelInput(userId: string, input: TravelInputData): void {
    const profile = UserProfileStorage.getUserProfile(userId);
    ...
    UserProfileStorage.saveUserProfile(userId, profile);
  }

  /**
   * Retrieves stored travel input data
   */
  public static getTravelInput(userId: string): TravelInputData | null {
    const profile = UserProfileStorage.getUserProfile(userId);
   ...
   */
  public static normalizeTravelInput(input: TravelInputData): TravelInputData {
    return {
   ...
   */
  public static calculateCompletenessScore(input: TravelInputData): number {
    let score = 0;
    let maxScore = 0;
    ...
    if (input.destinations && input.destinations.length > 0) {
      ...
      const specificDestinations = input.destinations.filter(dest =>
    ...
    if (input.experiences && input.experiences.length > 0) {
      ...
      const specificExperiences = input.experiences.filter(exp =>
        !['adventure', 'fun', 'experience', 'activity'].some(term =>
          exp.toLowerCase().includes(term) && exp.length < 15
        )
      );
    ...
    if (input.preferences) {
      ...
      if (input.preferences.travelStyle) score += 10;
