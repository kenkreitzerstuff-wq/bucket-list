...
exports.BucketListDataService = void 0;
/**
...
class BucketListDataService {
     ...
     */
    static getAllBucketListItems() {
        return this.ORIGINAL_BUCKET_LIST;
    }
    /**
     * Gets incomplete bucket list items (not done yet)
     */
    static getIncompleteBucketListItems() {
        return this.ORIGINAL_BUCKET_LIST.filter(item => !item.status || (!item.status.toLowerCase().includes('done')));
    }
    /**
     * Gets completed bucket list items
     */
    static getCompletedBucketListItems() {
        return this.ORIGINAL_BUCKET_LIST.filter(item => item.status && item.status.toLowerCase().includes('done'));
    }
    /**
     * Gets high priority items (Ken's priority 1-2 or Gail's HIGH interest)
     */
    static getHighPriorityItems() {
        return this.ORIGINAL_BUCKET_LIST.filter(item => item.kenPriority <= 2 || item.gailInterestLevel === 'HIGH');
    }
    /**
     * Gets items by priority level
     */
    static getItemsByPriority(priority) {
        return this.ORIGINAL_BUCKET_LIST.filter(item => item.kenPriority === priority);
    }
    /**
     * Gets items by Gail's interest level
     */
    static getItemsByGailInterest(level) {
    ...
    static getItemsByTags(tags) {
        return this.ORIGINAL_BUCKET_LIST.filter(item => tags.some(tag => item.tags.includes(tag.toLowerCase())));
    }
    /**
     * Gets items by difficulty level
     */
    static getItemsByDifficulty(difficulty) {
        return this.ORIGINAL_BUCKET_LIST.filter(item => item.difficulty === difficulty);
    }
    /**
     * Gets items suitable for a given duration
     */
    static getItemsByDuration(minDays, maxDays) {
        return this.ORIGINAL_BUCKET_LIST.filter(item => item.estimatedDuration >= minDays && item.estimatedDuration <= maxDays);
    }
    /**
     * Gets items suitable for current season
     */
    static getItemsForSeason(month) {
    ...
    static searchItems(query) {
        ...
        return this.ORIGINAL_BUCKET_LIST.filter(item => item.destination.toLowerCase().includes(lowerQuery) ||
            item.experiences.some((exp) => exp.toLowerCase().includes(lowerQuery)) ||
    ...
    static getRecommendedItems(preferences) {
        ...
        if (preferences.interests && preferences.interests.length > 0) {
            items = items.filter(item => preferences.interests.some(interest => item.tags.includes(interest.toLowerCase()) ||
                item.experiences.some((exp) => exp.toLowerCase().includes(interest.toLowerCase()))));
        }
        ...
        if (preferences.duration) {
            ...
            const range = durationMap[preferences.duration];
            if (range) {
                items = items.filter(item => item.estimatedDuration >= range.min && item.estimatedDuration <= range.max);
            }
        }
        ...
        items.sort((a, b) => {
            ...
            if (preferences.prioritizeGailInterest) {
                if (a.gailInterestLevel === 'HIGH' && b.gailInterestLevel !== 'HIGH')
                    return -1;
                if (b.gailInterestLevel === 'HIGH' && a.gailInterestLevel !== 'HIGH')
                    return 1;
            }
     ...
     */
    static toBucketItem(item, homeLocation) {
        return {
     ...
     */
    static estimateCost(item, homeLocation) {
     ...
     */
    static getSeasonFromMonth(month) {
        if (month >= 3 && month <= 5)
            return 'spring';
        if (month >= 6 && month <= 8)
            return 'summer';
     ...
     */
    static getTravelCompanyRecommendations() {
        return [
...
exports.BucketListDataService = BucketListDataService;
/**
 * Ken and Gail's original bucket list data from CSV
 */
BucketListDataService.ORIGINAL_BUCKET_LIST = [
    {
...
//# sourceMappingURL=BucketListDataService.js.map